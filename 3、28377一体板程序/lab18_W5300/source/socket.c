...
uint8    socket(SOCKET s, uint8 protocol, uint16 port, uint16 flag)
{
   ...
   if(protocol == Sn_MR_TCP){
     IINCHIP_WRITE(Sn_TTLR(s),128);   
     IINCHIP_WRITE(Sn_TOSR(s),0);   
     IINCHIP_WRITE(Sn_IMR(s),0x1F);  //    ÖÐ¶ÏÖÐ¶ÏÆÁ±Î
     IINCHIP_WRITE(Sn_PROTOR(s),0x100);  // 
	 IINCHIP_WRITE(IMR,0x0ff);
   }
...
uint8    connect(SOCKET s, uint8 * addr, uint16 port)
{
   ...
   //while()
	if( IINCHIP_READ(Sn_SSR(s)) != SOCK_SYNSENT )
	{
