...
Uint16   dat1[]={0,0,0,0,0,0,0};
Uint16   dat[]={ 0x00,0x11,0x05,0x06,0x04,0x11,0x20};//Uint16   dat[]={ 0x00,0x22,0x23,0x01,0x09,0x01,0x00};
Uint16   miao,fen,shi,ri,yue,nian;
...
// Step 4. Initialize the Device Peripherals:
   I2CA_Init();
  WriteData(dat,0x00,7);
   DELAY_US(10000);
   ReadData(dat1,0x00,7);
   // Application loop
  for(;;)
   {
      DELAY_US(1000*1000);
      ReadData(dat1,0x0,7);
      miao  =   (dat1[0]&15)+((dat1[0]>>4)&7)*10;
      fen   =   (dat1[1]&15)+((dat1[1]>>4)&7)*10;
      shi   =   (dat1[2]&15)+((dat1[2]>>4)&3)*10;
      ri    =   (dat1[4]&15)+((dat1[4]>>4)&3)*10;
      yue   =   (dat1[5]&15)+((dat1[5]>>4)&1)*10;
      nian  =   (dat1[6]&15)+((dat1[6]>>4)&15)*10;
   }   // end of for(;;)
}   // end of main
